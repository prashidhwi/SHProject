...
<script type="text/javascript">
        $(document).ready(function () {
            $('#invoiceTable').DataTable({
                ...
                'serverSide': true,
                'ajax': {
                    'url': '<%=context%>/invoice/list.do', // Endpoint in your Spring MVC controller
                    'type': 'POST',
                    'dataSrc': 'data' // Points to the "data" field in the server response
                },
                'columns': [
                    { 'data': 'invoiceNo' },
                    { 'data': 'customer' },
                    ...
                    { 'data': 'isPaid',
                    	'render': function(data, type, row){
                    		return '<input type="checkbox" id="isPaid" '+(data?'checked':'')+' onchange="updatePaymentStatus('+row.invoiceNo+',this.checked,event)" />'
                    	}
                    },
                    { 'data': 'invoiceDate',
                    	'render': function(data, type, row){
                    		const options = { year: 'numeric', month: 'numeric', day: 'numeric' };
                    		const date = new Date(data); // Convert the date string to a Date object
                            return date.toLocaleDateString('en-UK', options); // Format the date
                    	}
                    },
                    { 'data': 'receiptId' },
                    { 
                    	'data': 'note',
                    	'render': function(data, type, row){
                    		return '<button id="notes" onclick="alert(\''+data+'\')" class="sml-btn">Notes</button>'
                    	}	
                   	},
                   	{
                   		'data': 'invoiceNo',
                   		'render': function(data, type, row){
                   			return '<input type="button" value="Print" id="printInvoice" class="sml-btn" onclick="printInvoice('+data+')"/>';
                   		}
                   	},
                   	{
                   		'data': 'invoiceNo',
                   		'render': function(data, type, row){
                   			return '<a href="edit.do?invoiceNo='+data+'"><img alt="Edit" src="<%=context%>/images/icn_edit.png" /></a>';
                   		}
                   	},
                   	{
                   		'data': 'invoiceNo',
                   		'render': function(data, type, row){
                   			return '<a href="#" onclick="deleteInvoice('+data+')"><img alt="Delete" src="<%=context%>/images/icn_remove.png" width="24px" /></a>';
                   		}
                   	}
                ],
                'columnDefs' : [
        		...
        		"order": [[ 5, "desc" ]],
        		"responsive":true,
        		"autoWidth": false, // Prevents automatic column sizing to maintain 100% width
        	    "width": "100%"
            });
