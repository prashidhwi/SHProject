...
public class SupplierDao extends BaseJdbcDaoSupport {
	...
	public int save(Supplier supplier) {
		String sql = "insert into supplier(supplier_name,supplier_city,bill_id,amount,bill_date,status,input_date,input_user, note) values(?,?,?,?,str_to_date(?,'%d/%m/%Y'),1,sysdate(),?,?)";
		Object[] param = { supplier.getSupplierName(), supplier.getSupplierCity(), supplier.getBillId(),
				supplier.getAmount(), supplier.getBillDate(), getUser().getFullName(), supplier.getNote() };
		return getJdbcTemplate().update(sql, param);
	}

	public List<Supplier> getMatchingSupplier(String supplierName) {
		String sql = "select distinct supplier_name as supplierName, supplier_city as supplierCity from supplier where status>0 and upper(supplier_name) like upper('"
				+ supplierName + "%')";
	...
	public List<Supplier> getSupplierDetails(String supplierName, String supplierCity) {
		String sql = "select supplier_id as supplierId, bill_id as billId, amount, paid, bill_date as billDate, note from supplier where supplier_name=? and supplier_city=? and status>0";
		return getJdbcTemplate().query(sql, new BeanPropertyRowMapper<Supplier>(Supplier.class));
	}

	public List<Supplier> getBillsBySupplier(Supplier supplierBean) {
		String sql = "select supplier_id as supplierId, bill_id as billId, amount, paid, bill_date as invDate, note from supplier where supplier_name=? and supplier_city=? and status>0";
	...
	public int savePayment(SupplierPayment supplierPayment) {
		...
		Object[] param = { supplierPayment.getSupplierName(), supplierPayment.getSupplierCity(),
				supplierPayment.getAmount(), supplierPayment.getPaymentDate(), supplierPayment.getNote(),
				getUser().getFullName() };
		return getJdbcTemplate().update(sql, param);
	}
